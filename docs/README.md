# Bakery Dain Docs

이 폴더는 구현 전에 합의해야 할 제품, 디자인, 아키텍처, API, DB, 운영 원칙을 정리합니다.

## Product

- [Product Plan](product/product-plan.md)
- [KPI Plan](product/kpi-plan.md)
- [Roadmap](product/roadmap.md)
- [Slice 3 Reservation UX Shell](product/slice-3-reservation-ux.md)
- [Slice 4 Backend Skeleton](product/slice-4-backend-skeleton.md)
- [Slice 5 Data Layer](product/slice-5-data-layer.md)

## Design

- [Brand and Design Direction](design/brand-and-design.md)
- [UX Flows](design/ux-flows.md)

## Architecture

- [System Architecture](architecture/system-architecture.md)
- [Frontend Architecture](architecture/frontend-architecture.md)
- [Backend Architecture](architecture/backend-architecture.md)

## API and Data

- [API Contract Plan](api/api-contract-plan.md)
- [Database Plan](database/database-plan.md)

## Admin

- [Admin Plan](admin/admin-plan.md)

## Operations

- [Development Ground Rules](operations/development-ground-rules.md)
- [Delivery Plan](operations/delivery-plan.md)
- [Cloudflare Setup](operations/cloudflare-setup.md)
- [Risk Register](operations/risk-register.md)

## References

- [TereneHandover Reference Map](references/terene-handover-map.md)
